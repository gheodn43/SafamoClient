...
const ImageUpload = () => {
  ...
  const uploadImage = async () => {
    ...
    data.append(
      "upload_preset",
      process.env.hkqg4gwp
    );
    data.append("cloud_name", process.env.dxbx3fhiu);
    data.append("folder", "Cloudinary-React");

    try {
      const response = await fetch(
        `https://api.cloudinary.com/v1_1/${process.env.dxbx3fhiu}/image/upload`,
        {
          method: "POST",
          body: data,
        }
        ...
        {loading ? (
          ...
          url && (
            <div className="pb-8 pt-4">
              <Image
                cloudName={process.env.dxbx3fhiu}
                publicId={url}
              />
            </div>
          )
